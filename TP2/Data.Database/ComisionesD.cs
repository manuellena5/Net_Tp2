...
namespace Data.Database
{
    public class ComisionesD : Adapter
    {
        public List<Comisiones> GetAll()
        {
            ...
            {
                OpenConnection();
                SqlCommand cmdcomisiones = new SqlCommand("select cm.id_comision,cm.desc_comision,cm.anio_especialidad,cm.id_plan,pl.desc_plan from comisiones cm inner join planes pl on cm.id_plan=pl.id_plan ", SqlConn);
                ...
                while (drcomisiones.Read())
                {
                    ...
                    com.IdPlan = drcomisiones.IsDBNull(3) ? Convert.ToInt32(string.Empty) : ((int)drcomisiones["id_plan"]);
                    com.Plan = drcomisiones.IsDBNull(4) ? string.Empty : (string)drcomisiones["desc_plan"];
            ...
            {
                Exception ExcepcionManejada = new Exception("Error en cargar los datos", ex);
            }
            finally
            {
        ...
        public List<Business.Entities.Comisiones> GetByComision(string Tbuscado)
        {
            ...
            {
                this.OpenConnection();
                SqlCommand cmdcomision = new SqlCommand("select cm.id_comision,cm.desc_comision,cm.anio_especialidad,pl.desc_plan,pl.id_plan  from comisiones cm inner join planes pl on cm.id_plan=pl.id_plan where cm.desc_comision like @Tbuscado + '%'", SqlConn);
                ...
                while (drcomision.Read())
                {
                    ...
                    com.IdPlan = drcomision.IsDBNull(2) ? Convert.ToInt32(string.Empty) : ((int)drcomision["id_plan"]);
                    lista.Add(com);
        ...
        public Comisiones TraerUno(int id)
        {
            ...
            {
                this.OpenConnection();
                SqlCommand cmdcom = new SqlCommand("select com.id_comision,com.desc_comision,com.anio_especialidad,com.id_plan from comisiones com where com.id_comision = @id", SqlConn);
                cmdcom.Parameters.Add("@id", SqlDbType.Int).Value = id;
                SqlDataReader drcom = cmdcom.ExecuteReader();
                if (drcom.Read())
                {
                    com.IdComision = (int)drcom["id_comision"];
                    com.DescComision = (string)drcom["desc_comision"];
            ...
            {
                Exception ExcepcionManejada = new Exception("Error al buscar la comision", ex);
            }
            finally
            {
                this.CloseConnection();
        ...
        protected void Delete(Comisiones comision)
        {
            ...
            {
                Exception ExcepcionManejada = new Exception("Error al elimanar la Comision", Ex);
        ...
        protected void Update(Comisiones comision)
        {
            ...
            {
                this.OpenConnection();
                SqlCommand cmdSave = new SqlCommand("update comisiones set desc_comision=@desc_comision, anio_especialidad=@anio_especialidad,id_plan=@id_plan where id_comision=@id_comision", SqlConn);
